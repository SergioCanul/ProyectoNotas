...
<x-button-clr text="cancelar" :route="route('notas')"></x-button-clr>
  <form action="{{route('cornellnote.update',$nota->id)}}" method="POST" role="form" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 ">
    ...
    <div class="mb-4">
      ...
      <input class="shadow appearance-none border  rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline" id="ideas" name="ideas" type="text" placeholder="Escribe tus palabras clave.." value="{{$nota->ideas}}">
      @error('ideas')
    ...
    <div class="mb-2">
      ...
      <textarea class="shadow appearance-none border  rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline" id="resumen" name="resumen" type="text">{{$nota->resumen}}</textarea>
      @error('resumen')
        <div class="alert alert-danger mt-1 mb-1 text-red-700">{{ $message }}</div>
      @enderror
    </div>
